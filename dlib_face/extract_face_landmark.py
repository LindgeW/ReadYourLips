...
import cv2
import dlib
from imutils import face_utils  # imutils是基于opencv的图像处理工具包，对opencv进行了再次封装，简化了opencv的应用(图像处理操作)
...
def extract_face_landmarks(video_filename, predictor_params, refresh_size=8):
    ...
    predictor = dlib.shape_predictor(predictor_params)  # dat文件
    ...
    while cap.isOpened():
        ...
        if ret:
            ...
            if not (tracking_face and i < refresh_size):
                i = 0
                rects = detector(gray, 1)  # 放大倍数参数设为1，表明保持原始尺寸，直接在原图上进行检测
                if rects:
...
def save_face_landmarks(dataset_path, list_of_speakers, video_dir, dest_dir, predictor_params, file_ext='mpg',
                        refresh_size=8):
...
show_face_landmarks(r'D:\LipData\CMLR\video\s1\20170121\section_1_000.80_002.91.mp4',
                    r'D:\Codes\shape_predictor_68_face_landmarks.dat', refresh_size=8)
