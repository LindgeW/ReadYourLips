...
class CTCLipModel(nn.Module):
    ...
    def forward(self, vid, aud, tgt, vid_lens=None, aud_lens=None, tgt_lens=None):  # (b, t, c, h, w)
        ...
        #orth_loss = diff_loss(aud_id.unsqueeze(1).expand_as(enc_ac).detach(), enc_ac) + diff_loss(vid_id.unsqueeze(1).expand_as(enc_vc).detach(), enc_vc) + diff_loss(aud_id.unsqueeze(1).expand_as(enc_a).detach(), enc_a) + diff_loss(vid_id.unsqueeze(1).expand_as(enc_v).detach(), enc_v) + CORAL2(aud_id, vid_id)
        ...
        #orth_loss += diff_loss(enc_v, enc_vc) + diff_loss(enc_a, enc_ac) + diff_loss(enc_v.mean(1), enc_a.mean(1)) + CORAL2(enc_vc.mean(1), enc_ac.mean(1))
        ...
        #orth_loss += F.mse_loss(aud, recon_a) + F.mse_loss(vid, recon_v)
        #orth_loss += F.smooth_l1_loss(aud, recon_a) + F.smooth_l1_loss(vid, recon_v)

        enc_src = self.av_fusion(enc_ac, enc_vc, self.av_mlp)
        #enc_src_s = self.av_fusion(enc_ac, enc_vc, self.av_mlp)
        #enc_src_p = self.av_fusion(enc_a, enc_v, self.av_mlp2)
        #fusion_out = self.autofusion(torch.cat((enc_src_s, enc_src_p), dim=-1))
        #enc_src, auto_loss = fusion_out['z'], fusion_out['loss']
