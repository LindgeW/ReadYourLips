...
# https://github.com/yangyucheng000/ctcmodel/blob/main/src/metric.py
...
class LER(object):
    ...
    def update(self, *inputs):
        if len(inputs) != 3:
            raise ValueError('CRNNAccuracy need 3 inputs (y_pred, y, seq_len), but got {}'.format(len(inputs)))
    ...
    def eval(self):
        '''eval per epoch'''
        if self.length == 0:
            raise RuntimeError('Accuracy can not be calculated, because the number of length is 0.')
    ...
    def _prefix_beam_decode(self, y_pred, seq_len, beam_size=5):  # seq_len为输入特征的序列长度
