...
def procrustes_analysis2(shape, ref_shape):
    def get_rotation_matrix(theta):
        return np.array([[math.cos(theta), -math.sin(theta)],
                         [math.sin(theta), math.cos(theta)]])
...
def procrustes_analysis3(shape, ref_shape):
    def get_rotation_matrix(theta):
        return np.array([[math.cos(theta), -math.sin(theta)],
                         [math.sin(theta), math.cos(theta)]])
    ...
    a = np.dot(temp_sh, temp_ref.T).sum() / np.linalg.norm(temp_ref) ** 2
    b = np.sum(temp_sh[:, 0] * temp_ref[:, 1] - temp_ref[:, 0] * temp_sh[:, 1]) / np.linalg.norm(temp_ref) ** 2
    scale = np.sqrt(a ** 2 + b ** 2)
    theta = math.atan(b / max(a, 10 ** -10))
...
def test_trans_mat():
    ...
    M = transformation_from_points(x, y)
    xx = (M @ np.c_[x, np.ones(len(x))].T).T
    print('xx: ', xx, xx.shape)
    xxx = procrustes_analysis(x, y)
    print('xxx: ', xxx, xxx.shape)
