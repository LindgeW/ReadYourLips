...
class CTCLipModel(nn.Module):
    ...
    def forward(self, src, tgt, src_lens=None, tgt_lens=None):  # (b, t, c, h, w)
        b, t = src.size()[:2]
        src = self.visual_frontend_forward(src)
        vid_feat = src.reshape(b, -1, 512)
        #seq_feat = self.gru(vid_feat)[0]
        seq_feat = self.cfm(vid_feat, src_lens)
        ctc_out = self.fc(self.dropout(seq_feat))
        dec_out = self.trans_dec(tgt, seq_feat, src_lens, tgt_lens)
        #out2 = self.fc(F.normalize(seq_feat1, dim=-1)) + self.fc(F.normalize(seq_feat2, dim=-1))
        return ctc_out, dec_out, vid_feat, seq_feat

    def beam_search_decode(self, vids, lens, bos_id, eos_id, max_dec_len=80, pad_id=0):
        with torch.no_grad():
            b, t = vids.size()[:2]
            src = self.visual_frontend_forward(vids)
            vid_feat = src.reshape(b, -1, 512)
            enc_src = self.cfm(vid_feat, lens)
    ...
    def ctc_greedy_decode(self, vids, lens=None):
        with torch.no_grad():
            ...
            logits = self.fc(self.dropout(seq_feat))  # (B, T, V)
...
class DRLModel(nn.Module):
    ...
    def forward(self, vids, tgts, xlens, ylens):   # (B, T, C, H, W)
        ctc_logits, dec_logits = self.vsr(vids, tgts[:, :-1], xlens, ylens)[:2]
        ...
        vsr_loss = 0.8*attn_loss + 0.2*ctc_loss
        return vsr_loss
    ...
    def calc_orth_loss(self, vids, tgts, spk_ids, xlens, ylens):
        ...
        #diff_loss = self.diff_loss(s1.unsqueeze(1).detach(), c1) + self.diff_loss(s2.unsqueeze(1).detach(), c2)    # (N, D)  (N, T, D)
        diff_loss = self.diff_loss(spk_feat.unsqueeze(1).expand_as(cont_feat).detach(), cont_feat) 
    ...
    def diff_loss(self, x1, x2):  # (B, D1)  (B, D2)
        ...
        #dot_mat = torch.matmul(nx1, nx2.transpose(-1, -2))  # B x B
        #return torch.mean(dot_mat ** 2)
    ...
    def beam_decode(self, vids, src_lens, bos_id, eos_id, max_dec_len=50, pad_id=0):
        return self.vsr.beam_search_decode(vids, src_lens, bos_id, eos_id, max_dec_len, pad_id)
