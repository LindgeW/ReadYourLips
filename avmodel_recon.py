...
class CTCLipModel(nn.Module):
    def __init__(self, vocab_size, se=False):
        ...
        #    nn.Linear(512, 512)
        #    )
        self.spk_vid = nn.Linear(512*2, 512)
        self.spk_aid = nn.Linear(512*2, 512)
        ...
        self.aud_enc = nn.Sequential(
                ...
                nn.GELU(),
                nn.Linear(512, 512))
        self.recon_v = nn.Linear(512*3, 512)
        self.recon_a = nn.Linear(512*3, 512)
    ...
    def forward(self, vid, aud, tgt, vid_lens=None, aud_lens=None, tgt_lens=None):  # (b, t, c, h, w)
        ...
        #vid_mean, vid_std = torch.std_mean(vid, dim=1)
        #aud_mean, aud_std = torch.std_mean(aud, dim=1)
        #vid_id = self.spk_id(torch.cat((vid_mean, vid_std), dim=-1))   # x-vector
        #aud_id = self.spk_id(torch.cat((aud_mean, aud_std), dim=-1))   # x-vector
        ...
        enc_v0, enc_a0 = self.cfm_v(vid, vid_lens), self.cfm_a(aud, aud_lens)
        enc_v, enc_a = self.vid_enc(enc_v0), self.aud_enc(enc_a0)
        enc_vc, enc_ac = self.share_enc(enc_v0), self.share_enc(enc_a0)

        vid_mean, vid_std = torch.std_mean(enc_v0, dim=1)
        aud_mean, aud_std = torch.std_mean(enc_a0, dim=1)
        ...
        #orth_loss = diff_loss(aud_id.unsqueeze(1).expand_as(enc_ac), enc_ac) + diff_loss(vid_id.unsqueeze(1).expand_as(enc_vc), enc_vc) + diff_loss(aud_id.unsqueeze(1).expand_as(enc_a), enc_a) + diff_loss(vid_id.unsqueeze(1).expand_as(enc_v), enc_v) + F.mse_loss(aud_id, vid_id)
        ...
        #orth_loss = diff_loss(aud_id.unsqueeze(1).expand_as(enc_ac), enc_ac) + diff_loss(vid_id.unsqueeze(1).expand_as(enc_vc), enc_vc) + diff_loss(aud_id.unsqueeze(1).expand_as(enc_a), enc_a) + diff_loss(vid_id.unsqueeze(1).expand_as(enc_v), enc_v) + cmd(aud_id, vid_id)
        #orth_loss = diff_loss(aud_id.unsqueeze(1).expand_as(enc_ac), enc_ac) + diff_loss(vid_id.unsqueeze(1).expand_as(enc_vc), enc_vc) + diff_loss(aud_id.unsqueeze(1).expand_as(enc_a), enc_a) + diff_loss(vid_id.unsqueeze(1).expand_as(enc_v), enc_v) + CORAL3(aud_id, vid_id)
        ...
        # 模态无关
        orth_loss += ranking_loss(enc_vc.mean(1, keepdims=True), enc_v, enc_ac.mean(1, keepdims=True), enc_a) + diff_loss(enc_v.mean(1), enc_a.mean(1))
        #orth_loss += diff_loss(enc_v, enc_vc) + diff_loss(enc_a, enc_ac) + diff_loss(enc_v.mean(1), enc_a.mean(1)) + F.mse_loss(enc_vc.mean(1), enc_ac.mean(1))
        #orth_loss += diff_loss(enc_v, enc_vc) + diff_loss(enc_a, enc_ac) + diff_loss(enc_v.mean(1), enc_a.mean(1)) + F.smooth_l1_loss(enc_vc.mean(1), enc_ac.mean(1))
        #orth_loss += diff_loss(enc_v, enc_vc) + diff_loss(enc_a, enc_ac) + diff_loss(enc_v.mean(1), enc_a.mean(1)) + cmd(enc_vc.mean(1), enc_ac.mean(1))
        #orth_loss += diff_loss(enc_v, enc_vc) + diff_loss(enc_a, enc_ac) + diff_loss(enc_v.mean(1), enc_a.mean(1)) + CORAL3(enc_vc.mean(1), enc_ac.mean(1))
        #orth_loss += diff_loss(enc_v, enc_vc) + diff_loss(enc_a, enc_ac) + diff_loss(enc_v.mean(1), enc_a.mean(1)) + mmd(enc_vc.mean(1), enc_ac.mean(1))
        
        # 模态重构
        #recon_v, recon_a = self.recon_v(enc_v + enc_vc + vid_id.unsqueeze(1)), self.recon_a(enc_a + enc_ac + aud_id.unsqueeze(1))
        ...
        orth_loss += F.mse_loss(enc_v0, recon_v) + F.mse_loss(enc_a0, recon_a) + F.mse_loss(enc_v, self.vid_enc(recon_v)) + F.mse_loss(enc_a, self.aud_enc(recon_a))
