...
class CTCLipModel(nn.Module):
    def __init__(self, vocab_size, num_spk):
        ...
        self.trans_dec = TransDecoder(vocab_size, 768, 6, 4) 
    ...
    def forward(self, vid, aud, tgt, vid_lens=None, aud_lens=None, tgt_lens=None):  # (b, t, c, h, w)
        ...
        '''
        enc_src1 = self.avhubert(None, aud.transpose(1, 2))   # ASR: (b, c, t)
        attn_out1 = self.trans_dec(tgt[:, :-1], enc_src1, aud_lens, tgt_lens)
        ctc_log_probs1 = self.fc(enc_src1).log_softmax(dim=-1).transpose(0, 1)  # (T, B, V)
        ctc_loss1 = F.ctc_loss(ctc_log_probs1, tgt[:, 1:], aud_lens.reshape(-1), tgt_lens.reshape(-1), zero_infinity=True) 
        attn_loss1 = F.cross_entropy(attn_out1.transpose(-1, -2).contiguous(), tgt[:, 1:], ignore_index=0)
            
        enc_src2 = self.avhubert(vid.transpose(1, 2), None)   # VSR: (b, c, t, h, w) 
        attn_out2 = self.trans_dec(tgt[:, :-1], enc_src2, vid_lens, tgt_lens)
        ctc_log_probs2 = self.fc(enc_src2).log_softmax(dim=-1).transpose(0, 1)  # (T, B, V)
        ctc_loss2 = F.ctc_loss(ctc_log_probs2, tgt[:, 1:], vid_lens.reshape(-1), tgt_lens.reshape(-1), zero_infinity=True) 
        attn_loss2 = F.cross_entropy(attn_out2.transpose(-1, -2).contiguous(), tgt[:, 1:], ignore_index=0)
       
        ctc_loss = ctc_loss1 + ctc_loss2
        attn_loss = attn_loss1 + attn_loss2
        '''
        enc_src = self.avhubert(vid.transpose(1, 2), None)   # VSR: (b, c, t, h, w) 
        attn_out = self.trans_dec(tgt[:, :-1], enc_src, vid_lens, tgt_lens)
        ctc_log_probs = self.fc(enc_src).log_softmax(dim=-1).transpose(0, 1)  # (T, B, V)
        ctc_loss = F.ctc_loss(ctc_log_probs, tgt[:, 1:], vid_lens.reshape(-1), tgt_lens.reshape(-1), zero_infinity=True) 
        attn_loss = F.cross_entropy(attn_out.transpose(-1, -2).contiguous(), tgt[:, 1:], ignore_index=0)
        return ctc_loss, attn_loss
    ...
    def beam_search_decode(self, vid, aud, vid_lens, aud_lens, bos_id, eos_id, max_dec_len=80, pad_id=0):
        with torch.no_grad():
            ...
            res = beam_decode(self.trans_dec, enc_src, vid_lens, bos_id, eos_id, max_output_length=max_dec_len, beam_size=10)
            return res.detach().cpu()
            #return beam_decode(self.trans_dec, enc_src, vid_lens, bos_id, eos_id, max_output_length=max_dec_len, beam_size=40, n_best=40)
...
class DRLModel(nn.Module):
    ...
    def forward(self, vids, auds, tgts, spk_ids, vid_lens, aud_lens, tgt_lens):   # (B, T, C, H, W)
        ...
        #ctc_logits, attn_logits, spk_logits, drl_loss = self.avsr(vids, auds, tgts[:, :-1], vid_lens, aud_lens, tgt_lens)[:4]
        #spk_loss = F.cross_entropy(spk_logits, spk_ids, ignore_index=-1)
        
        #ctc_logits, attn_logits = self.avsr(vids, auds, tgts[:, :-1], vid_lens, aud_lens, tgt_lens)
        ...
        #attn_loss = F.cross_entropy(attn_logits.transpose(-1, -2).contiguous(), tgts[:, 1:], ignore_index=0)
        
        ctc_loss, attn_loss = self.avsr(vids, auds, tgts, vid_lens, aud_lens, tgt_lens)
        
        #vsr_loss = 0.9*attn_loss + 0.1*ctc_loss  # wer=40
        vsr_loss = attn_loss + 0.1*ctc_loss  # wer=32  vsrapp3.pt
    ...
    def calc_orth_loss(self, vids, tgts, spk_ids, xlens, ylens):
        ...
        ctc_logits, attn_logits, _, cont_feat = self.vsr(vids, tgts[:, :-1], xlens, ylens)
        ...
        attn_loss = F.cross_entropy(attn_logits.transpose(-1, -2).contiguous(), tgts[:, 1:].long(), ignore_index=0)
